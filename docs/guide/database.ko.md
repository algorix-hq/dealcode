...
!!! note "범위 모드: 정수 코드, 평범한 유일성"

    [정수 범위 모드](configuration.ko.md#integer-range-mode)에서는 코드가
    정수이고 사이클도 없어서 기본 스키마가 그대로 적용됩니다 — 코드
    컬럼을 정수 컬럼으로 둬도 됩니다(그게 이 모드의 존재 이유입니다).
    챙길 것 두 가지: 발급 가능 개수는 `high − low + 1`이 아니라
    `codec.capacity`이므로 카운터가 거기 닿기 전에 경보를 걸 것, 그리고
    `UNIQUE` 인덱스는 위와 똑같이 경보 장치로 유지할 것.
